...
void Foam::fvsc::GaussVolPointBase3D::triCalcWeights(const fvMesh& mesh)
{
    ...
    forAll(tf_, i)
    {
        ...
        /* Coefficients for X */
        atx_[i].resize(5);
        atx_[i][0] = OneBySix * ((mesh.C()[own].z() - mesh.C()[nei].z()) *
                                 (points[p2].y() - points[p3].y()) +
                                 (mesh.C()[nei].y() - mesh.C()[own].y()) * (points[p2].z() - points[p3].z()));
        atx_[i][1] = OneBySix * ((mesh.C()[nei].y() - mesh.C()[own].y()) *
                                 (points[p3].z() - points[p1].z()) +
                                 (mesh.C()[own].z() - mesh.C()[nei].z()) * (points[p3].y() - points[p1].y()));
        atx_[i][2] = OneBySix * ((mesh.C()[nei].y() - mesh.C()[own].y()) *
        ...
        /* Coefficients for Y */
        aty_[i].resize(5);
        aty_[i][0] = OneBySix * ((mesh.C()[own].x() - mesh.C()[nei].x()) *
                                 (points[p2].z() - points[p3].z()) +
                                 (mesh.C()[nei].z() - mesh.C()[own].z()) * (points[p2].x() - points[p3].x()));
        aty_[i][1] = OneBySix * ((mesh.C()[nei].z() - mesh.C()[own].z()) *
                                 (points[p3].x() - points[p1].x()) +
                                 (mesh.C()[own].x() - mesh.C()[nei].x()) * (points[p3].z() - points[p1].z()));
        aty_[i][2] = OneBySix * ((mesh.C()[nei].z() - mesh.C()[own].z()) *
        ...
        /* Coefficients for Z */
        atz_[i].resize(5);
        atz_[i][0] = OneBySix * ((mesh.C()[own].y() - mesh.C()[nei].y()) *
                                 (points[p2].x() - points[p3].x()) +
                                 (mesh.C()[nei].x() - mesh.C()[own].x()) * (points[p2].y() - points[p3].y()));
        atz_[i][1] = OneBySix * ((mesh.C()[nei].x() - mesh.C()[own].x()) *
                                 (points[p3].y() - points[p1].y()) +
                                 (mesh.C()[own].y() - mesh.C()[nei].y()) * (points[p3].x() - points[p1].x()));
        atz_[i][2] = OneBySix * ((mesh.C()[nei].x() - mesh.C()[own].x()) *
    ...
    forAll(btf_, iPatch)
    {
        ...
        forAll(btf_[iPatch], k)
        {
            ...
            batx_[iPatch][k][0] = OneBySix * ((v5[facei].z() - v4[facei].z()) *
                                              (points[p2].y() - points[p3].y()) +
                                              (v4[facei].y() - v5[facei].y()) * (points[p2].z() - points[p3].z()));
            batx_[iPatch][k][1] = OneBySix * ((v4[facei].y() - v5[facei].y()) *
                                              (points[p3].z() - points[p1].z()) +
                                              (v5[facei].z() - v4[facei].z()) * (points[p3].y() - points[p1].y()));
            batx_[iPatch][k][2] = OneBySix * ((v4[facei].y() - v5[facei].y()) *
                                              (points[p1].z() - points[p2].z()) +
                                              (v5[facei].z() - v4[facei].z()) * (points[p1].y() - points[p2].y()));
            batx_[iPatch][k][3] = OneBySix * (points[p1].z() * (points[p2].y() -
                                              points[p3].y()) +
                                              points[p2].z() * (points[p3].y() - points[p1].y()) +
                                              points[p3].z() * (points[p1].y() - points[p2].y()));
            ...
            baty_[iPatch][k][0] = OneBySix * ((v5[facei].x() - v4[facei].x()) *
                                              (points[p2].z() - points[p3].z()) +
                                              (v4[facei].z() - v5[facei].z()) * (points[p2].x() - points[p3].x()));
            baty_[iPatch][k][1] = OneBySix * ((v4[facei].z() - v5[facei].z()) *
                                              (points[p3].x() - points[p1].x()) +
                                              (v5[facei].x() - v4[facei].x()) * (points[p3].z() - points[p1].z()));
            baty_[iPatch][k][2] = OneBySix * ((v4[facei].z() - v5[facei].z()) *
                                              (points[p1].x() - points[p2].x()) +
                                              (v5[facei].x() - v4[facei].x()) * (points[p1].z() - points[p2].z()));
            baty_[iPatch][k][3] = OneBySix * (points[p1].x() * (points[p2].z() -
                                              points[p3].z()) +
                                              points[p2].x() * (points[p3].z() - points[p1].z()) +
                                              points[p3].x() * (points[p1].z() - points[p2].z()));
            ...
            batz_[iPatch][k][0] = OneBySix * ((v5[facei].y() - v4[facei].y()) *
                                              (points[p2].x() - points[p3].x()) +
                                              (v4[facei].x() - v5[facei].x()) * (points[p2].y() - points[p3].y()));
            batz_[iPatch][k][1] = OneBySix * ((v4[facei].x() - v5[facei].x()) *
                                              (points[p3].y() - points[p1].y()) +
                                              (v5[facei].y() - v4[facei].y()) * (points[p3].x() - points[p1].x()));
            batz_[iPatch][k][2] = OneBySix * ((v4[facei].x() - v5[facei].x()) *
                                              (points[p1].y() - points[p2].y()) +
                                              (v5[facei].y() - v4[facei].y()) * (points[p1].x() - points[p2].x()));
            batz_[iPatch][k][3] = OneBySix * (points[p1].y() * (points[p2].x() -
                                              points[p3].x()) +
                                              points[p2].y() * (points[p3].x() - points[p1].x()) +
                                              points[p3].y() * (points[p1].x() - points[p2].x()));
...
void Foam::fvsc::GaussVolPointBase3D::quaCalcWeights(const fvMesh& mesh)
{
    ...
    forAll(qf_, i)
    {
        ...
        /* coefficient for X */
        aqx_[i].resize(6);
        aqx_[i][0] = OneBySix * ((mesh.C()[nei].y() - mesh.C()[own].y()) *
                                 (points[p2].z() - points[p4].z())
                                 - (mesh.C()[nei].z() - mesh.C()[own].z()) * (points[p2].y() - points[p4].y()));
        aqx_[i][1] = OneBySix * ((mesh.C()[nei].y() - mesh.C()[own].y()) *
                                 (points[p3].z() - points[p1].z())
                                 - (mesh.C()[nei].z() - mesh.C()[own].z()) * (points[p3].y() - points[p1].y()));
        aqx_[i][5] = OneBySix * ((points[p1].y() - points[p3].y()) *
                                 (points[p2].z() - points[p4].z())
        ...
        /* coefficient for Y */
        aqy_[i].resize(6);
        aqy_[i][0] = OneBySix * ((mesh.C()[nei].z() - mesh.C()[own].z()) *
                                 (points[p2].x() - points[p4].x())
                                 - (mesh.C()[nei].x() - mesh.C()[own].x()) * (points[p2].z() - points[p4].z()));
        aqy_[i][1] = OneBySix * ((mesh.C()[nei].z() - mesh.C()[own].z()) *
                                 (points[p3].x() - points[p1].x())
                                 - (mesh.C()[nei].x() - mesh.C()[own].x()) * (points[p3].z() - points[p1].z()));
        aqy_[i][5] = OneBySix * ((points[p1].z() - points[p3].z()) *
                                 (points[p2].x() - points[p4].x())
        ...
        /* coefficient for Z */
        aqz_[i].resize(6);
        aqz_[i][0] = OneBySix * ((mesh.C()[nei].x() - mesh.C()[own].x()) *
                                 (points[p2].y() - points[p4].y())
                                 - (mesh.C()[nei].y() - mesh.C()[own].y()) * (points[p2].x() - points[p4].x()));
        aqz_[i][1] = OneBySix * ((mesh.C()[nei].x() - mesh.C()[own].x()) *
                                 (points[p3].y() - points[p1].y())
                                 - (mesh.C()[nei].y() - mesh.C()[own].y()) * (points[p3].x() - points[p1].x()));
        aqz_[i][5] = OneBySix * ((points[p1].x() - points[p3].x()) *
                                 (points[p2].y() - points[p4].y())
    ...
    forAll(bqf_, iPatch)
    {
        ...
        forAll(bqf_[iPatch], k)
        {
            ...
            baqx_[iPatch][k][0] = OneBySix * ((v5[facei].y() - v6[facei].y()) *
                                              (points[p2].z() - points[p4].z())
                                              - (v5[facei].z() - v6[facei].z()) * (points[p2].y() - points[p4].y()));
            baqx_[iPatch][k][1] = OneBySix * ((v5[facei].y() - v6[facei].y()) *
                                              (points[p3].z() - points[p1].z())
                                              - (v5[facei].z() - v6[facei].z()) * (points[p3].y() - points[p1].y()));
            baqx_[iPatch][k][5] = OneBySix * ((points[p1].y() - points[p3].y()) *
                                              (points[p2].z() - points[p4].z())
                                              - (points[p1].z() - points[p3].z()) * (points[p2].y() - points[p4].y()));
            ...
            baqy_[iPatch][k][0] = OneBySix * ((v5[facei].z() - v6[facei].z()) *
                                              (points[p2].x() - points[p4].x())
                                              - (v5[facei].x() - v6[facei].x()) * (points[p2].z() - points[p4].z()));
            baqy_[iPatch][k][1] = OneBySix * ((v5[facei].z() - v6[facei].z()) *
                                              (points[p3].x() - points[p1].x())
                                              - (v5[facei].x() - v6[facei].x()) * (points[p3].z() - points[p1].z()));
            baqy_[iPatch][k][5] = OneBySix * ((points[p1].z() - points[p3].z()) *
                                              (points[p2].x() - points[p4].x())
                                              - (points[p1].x() - points[p3].x()) * (points[p2].z() - points[p4].z()));
            ...
            baqz_[iPatch][k][0] = OneBySix * ((v5[facei].x() - v6[facei].x()) *
                                              (points[p2].y() - points[p4].y())
                                              - (v5[facei].y() - v6[facei].y()) * (points[p2].x() - points[p4].x()));
            baqz_[iPatch][k][1] = OneBySix * ((v5[facei].x() - v6[facei].x()) *
                                              (points[p3].y() - points[p1].y())
                                              - (v5[facei].y() - v6[facei].y()) * (points[p3].x() - points[p1].x()));
            baqz_[iPatch][k][5] = OneBySix * ((points[p1].x() - points[p3].x()) *
                                              (points[p2].y() - points[p4].y())
                                              - (points[p1].y() - points[p3].y()) * (points[p2].x() - points[p4].x()));
...
void Foam::fvsc::GaussVolPointBase3D::faceGrad(const volScalarField& sf,
        surfaceVectorField& gradf)
{
    pointScalarField pF
    (
...
void Foam::fvsc::GaussVolPointBase3D::faceGrad(const volVectorField& vf,
        surfaceTensorField& gradf)
{
    pointVectorField pF
    (
...
void Foam::fvsc::GaussVolPointBase3D::faceDiv(const volVectorField& vf,
        surfaceScalarField& divf)
{
    pointVectorField pF
    (
...
void Foam::fvsc::GaussVolPointBase3D::faceDiv(const volTensorField& tf,
        surfaceVectorField& divf)
{
    pointTensorField pF
    (
